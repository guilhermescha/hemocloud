...
public class UserReadConverter implements Converter<DBObject, User> {
	public User convert(DBObject source) {
		return new User((String) source.get("username"), (String) source.get("password"),
				convertAuthoritiesReading(source));
	}

	private Collection<? extends GrantedAuthority> convertAuthoritiesReading(DBObject source) {
